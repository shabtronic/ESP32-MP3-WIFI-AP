...
void i2c_master_init(int sda, int scl)
	...
	//i2c_set_period(I2C_NUM_0, 12, 24);
	//(i2c_set_start_timing(I2C_NUM_0, 50, 60));
	//(i2c_set_data_timing(I2C_NUM_0, 80, 60));
	//(i2c_set_stop_timing(I2C_NUM_0, 100, 60));
...
bool OLED::i2c_stop()
	...
	bool res = true;
	i2c_master_stop(cmd);
	//if (i2c_master_cmd_begin(I2C_NUM_0, cmd, 1 / portTICK_PERIOD_MS) != ESP_OK)
	//	res = false;
	if (i2c_master_cmd_begin(I2C_NUM_0, cmd, I2C_MASTER_LAST_NACK) != ESP_OK)
		res = false;
...
bool OLED::i2c_send(uint8_t byte)
	{
	i2c_master_write_byte(cmd, byte, false);
	return false;
	}
...
bool OLED::SSD1306_init(int i2c_address, int vccstate)
	{

	i2c_start();
	i2c_send(i2c_address << 1);
	i2c_send(SSD1306_DISPLAYOFF);
	i2c_send(SSD1306_SETDISPLAYCLOCKDIV);
	i2c_send(0x80);

	i2c_send(SSD1306_SETMULTIPLEX);
	i2c_send(height - 1);

	i2c_send(SSD1306_SETDISPLAYOFFSET);
	i2c_send(0x0);
	i2c_send(SSD1306_SETSTARTLINE | 0x0);
	i2c_send(SSD1306_CHARGEPUMP);

	i2c_send((vccstate == SSD1306_EXTERNALVCC) ? 0x10 : 0x14);

	i2c_send(SSD1306_MEMORYMODE);
	i2c_send(0x00);
	i2c_send(SSD1306_SEGREMAP | 0x1);
	i2c_send(SSD1306_COMSCANDEC);

	i2c_send(SSD1306_SETCOMPINS);
	i2c_send(0x12);
	i2c_send(SSD1306_SETCONTRAST);

	i2c_send((vccstate == SSD1306_EXTERNALVCC) ? 0x9F : 0xCF);

	i2c_send(SSD1306_SETPRECHARGE);
	i2c_send((vccstate == SSD1306_EXTERNALVCC) ? 0x22 : 0xF1);

	i2c_send(SSD1306_SETVCOMDETECT);
	i2c_send(0x40);
	i2c_send(SSD1306_DISPLAYALLON_RESUME);
	i2c_send(SSD1306_NORMALDISPLAY);
	i2c_send(SSD1306_DEACTIVATE_SCROLL);
	i2c_send(SSD1306_DISPLAYON);
	return i2c_stop();
	}
...
void OLED::SSD1306_display(void)
	{
	i2c_start();
	i2c_send(i2c_address << 1);

	i2c_send(SSD1306_PAGEADDR);
	i2c_send(0);
	i2c_send(0xFF);
	i2c_send(SSD1306_COLUMNADDR);
	i2c_send(0);
	i2c_send(width - 1);
...
OLED::OLED(uint8_t i2c_address, int sda, int sdl, uint_fast8_t width, uint_fast8_t height, tDriver drive) :
...
bool OLED::begin()
	{
	delay(100);
	if (Driver == tDriver::SSD1306)
		return SSD1306_init(i2c_address, 0);
...
void OLED::draw_byte(int x, int y, uint8_t b, tColor color)
	{
	for (int a = 0; a < 8; a++)
		{
		if (b & 1)
...
void OLED::draw_bytes(int x, int y, const uint8_t* data, uint_fast8_t size, tSize scaling, tColor color, bool useProgmem)
	{
...
size_t OLED::draw_character(int x, int y, char c, tSize scaling, tColor color)
	{
	// Invalid position
...
void OLED::draw_string(int x, int y, const char* s, tSize scaling, tColor color)
	{
	while (*s)
		{
...
void OLED::draw_string_P(int x, int y, const char* s, tSize scaling, tColor color)
	{
	while (*s)
		{
		draw_character(x, y, *s++, scaling, color);
...
void OLED::draw_pixel(int x, int y, tColor color)
	{
	//	y = 63 - y;
...
void OLED::draw_line(int x0, int y0, int x1, int y1, tColor color)
	{
...
size_t OLED::printf(int x, int y, const char *format, ...) {
	va_list arg;
	va_start(arg, format);
	char temp[64];
	char* buffer = temp;
