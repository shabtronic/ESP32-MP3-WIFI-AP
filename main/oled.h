...
class OLED
	...
	public:
		...
		OLED(uint8_t i2c_address = 0x3C,int sda=22,int sdl=21, uint_fast8_t width = 128, uint_fast8_t height = 32, tDriver drive=tDriver::SH1106);
		...
		inline void draw_pixel(int x, int y, tColor color = WHITE);
		void draw_line(int x0, int y0, int x1, int y1, tColor color = WHITE);
		...
		size_t draw_character(int x, int y, char c, tSize scaling = NORMAL_SIZE, tColor color = WHITE);
		void draw_string(int x, int y, const char* s, tSize scaling = NORMAL_SIZE, tColor color = WHITE);
		void draw_string_P(int x, int y, const char* s, tSize scaling = NORMAL_SIZE, tColor color = WHITE);
		void setCursor(uint_fast8_t x, uint_fast8_t y);
		size_t printf(int x, int y, const char *format, ...);
	...
	private:
		...
		void draw_byte(int x, int y, uint8_t b, tColor color);
		void draw_bytes(int x, int y, const uint8_t* data, uint_fast8_t size, tSize scaling, tColor color, bool useProgmem);
		void SSD1306_display(void);
		bool SSD1306_init(int i2c_address, int vccstate);
		// ESP32 stuff
		void i2c_start();
		bool i2c_stop();
